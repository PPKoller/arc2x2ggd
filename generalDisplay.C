...
//============================================
//============================================
void generalDisplay( TString filename, Bool_t drawbeam=kTRUE, Int_t checkoverlaps=0, Int_t vislevel=20 )
{
  TEveManager::Create();
...
void paintingVolumes( TGeoManager *geo )
{
  ...
  std::cout << nvolumes << std::endl;
  for ( int i = 0; i < nvolumes; i++ )
  {
    volume = (TGeoVolume*)volumes->At(i);
    volume->SetVisContainers(kTRUE);

     std::cout << "vol " << TString(volume->GetName()) << std::endl;

    if ( TString(volume->GetName()) == "world_vol")
    {
      std::cout << "sadfasfdasfa\n";
      volume->SetInvisible();
      continue;
    }
    /*if ( TString(volume->GetName()) == "_vol")
    {
      std::cout << "sadfasfdasfa\n";
